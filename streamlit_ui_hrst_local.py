...
if st.session_state["authentication_status"]:
    ...
    if menu_id == "Home":
            ...
            df_til = data_upload()
                        ...
                        # Create a dictionary to map severities to background colors, font colors, and box sizes
            severity_styles = {
                'Warning': {'background_color': 'yellow', 'font_color': 'black', 'box_size': '170px'},
                'Average': {'background_color': 'orange', 'font_color': 'black', 'box_size': '170px'},
                'High': {'background_color': 'red', 'font_color': 'white', 'box_size': '170px'},
                'Disaster': {'background_color': 'darkred', 'font_color': 'white', 'box_size': '170px'},
                'Information': {'background_color': 'blue', 'font_color': 'white', 'box_size': '170px'},
                'Not Classified': {'background_color': 'lightgrey', 'font_color': 'black', 'box_size': '200px'}
            }

            # Calculate the occurrences of each severity description
            severity_counts = df_til['Severity_Description'].value_counts()

            # Create columns for displaying severity descriptions and their values
            col6, col7, col8, col9, col10, col11 = st.columns(6)

            for i, (severity, count) in enumerate(severity_counts.items()):
                # Choose a column to display the severity-description-value pair
                if i % 6 == 0:
                    current_col = col6
                elif i % 6 == 1:
                    current_col = col7
                ...
                # Get the background color, font color, and box size for the severity
                styles = severity_styles.get(severity, {'background_color': 'lightgrey', 'font_color': 'black', 'box_size': '170px'})

                # Display the severity-description-value pair in a styled box with center-aligned severity
                with current_col:
                    st.markdown(f'<div style="border: 2px solid black; background-color: {styles["background_color"]}; '
                                f'box-shadow: 2px 2px 5px rgba(0, 0, 0, 0.2); padding: 1px;text-align: center; color: {styles["font_color"]}; width: {styles["box_size"]};">'
                                f'<h5 style="color: {styles["font_color"]}">{severity}</h5>'
                                f'<p><b>Value:</b> {count}</p>'
            ...
            # col8.title("Ping Response Time and Loss")
            # col7.title("Host Problems")

            col7.write("")
            col8.write("")
            result_placeholder_time = col8.empty()
            result_placeholder_loss = col8.empty()
            chart_placeholder = col8.altair_chart(chart, use_container_width=True)
